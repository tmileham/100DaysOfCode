...
def calculate_score(card_list):
        """ returns sum of cards. if sum is 21 returns 0 """
        if len(card_list) == 2 and sum(card_list) == 21: # Blackjack return score as int 0
            return 0
        if sum(card_list) > 21:
            if 11 in card_list:
                card_list.remove(11)
...
def compare(player_score,computer_score):
    """Returns a string with result of score comparison"""
    if computer_score == 0:
        return "Dealer wins"
    elif player_score == 0:
        return "Player wins!"
        ...
        return "You won. Dealer went bust!"
    elif player_score == computer_score:
        return "Its a draw!"
    elif player_score > computer_score:
        return "You won! you had a higher score"
    else:
        return "Dealer won, their score was higher"
...
def play_game():
    ...
    while not is_game_over:
        ...
        print(f"The dealers first card {computer_cards[0]}")
...
#Hint 10: If the game has not ended, ask the user if they want to draw another card. If yes, then use the deal_card() function to add another 
